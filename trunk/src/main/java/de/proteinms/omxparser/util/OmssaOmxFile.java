...
import de.proteinms.omxparser.util.omssaparser.MSSearch;
...
public class OmssaOmxFile {
	 ...
	 * <br><br>Note: To understand the structure of the MSSearch and child objects it is very helpful to read 
	 * "OMSSA.mod.dtd" and "OMSSA.xsd"
	 * @return MSSearch
	 */
	 ...
	 * <br><br>Note: In this HashMap the Peptides are only represented through their sequences which are stored as a HashSet of Strings. For further information
	 * about the Peptide ("at what position in the corresponding protein does the Peptide start/end?") you´ll need to search 
	 * in the MSPepHit object.
	 ...
	 * <br><br>Note: In this case the Proteins are represented by a MSPepHit object. This object stores
	 * among other things the position of the Peptide in the Protein.
	 ...
	 * given Protein (param, has to be the accession of the Protein). This HashMap contains valuable information about the location of the Peptide in the Protein etc.
	...
	private void processSpectrumToHitSetMap(MSSearch result){
		...
		for (int i=0; i<result.MSSearch_request.MSRequest.size();i++){
			for (Integer key: result.MSSearch_request.MSRequest.get(i).MSRequest_spectra.MSSpectrumset.MSSpectrum.keySet()){
				spectrumToHitSetMap.put(result.MSSearch_request.MSRequest.get(i).MSRequest_spectra.MSSpectrumset.MSSpectrum.get(key), result.MSSearch_response.MSResponse.get(i).MSResponse_hitsets.MSHitSet.get(key));
